...
## Topic:     JUN 2024 | make a funnel chart
## Author:    Steven Ponce
## Date:      2024-06-02
...
## Data:      From Discovery to Commercialization  
##            Synthetic data generated using ChatGPT 4
...
pacman::p_load(
  ...
  ggfunnel     # A 'R' package to create 'Power BI' like funnel charts.
)
...
data <- tibble(
  stage = c(
    "Target Identification & Validation", "Compound Screening",
    "Hit Validation", "Lead Identification & Optimization",
    "Clinical Trials & FDA Approval", "Commercialization"
  ),
  molecules = c(1000000, 10000, 250, 50, 5, 1),
  label_x = c(0,0.06,0.02,0.02,0.02,0.02),
  label_y = c(6, 5, 4, 3, 2, 1)
) |> 
  mutate(
    stage = str_wrap(stage, width = 15),
    hjust_value = if_else(stage == "Target Identification & Validation", 0.5, 1.5)  
  )
...
### |-  titles and caption ----
# icons
tt <- str_glue("#SWDchallenge: June 2024 &bull; Source: Synthetic Data (generated using ChatGPT 4)<br>")  
li <- str_glue("<span style='font-family:fa6-brands'>&#xf08c;</span>")  
gh <- str_glue("<span style='font-family:fa6-brands'>&#xf09b;</span>")
mn <- str_glue("<span style='font-family:fa6-brands'>&#xf4f6;</span>")

# text
title_text   <- str_glue("Drug Development Funnel") 

subtitle_text <- str_glue("Stages of drug development from discovery to market<br>
                          <span style='font-size:8pt; color:gray30'>__Note__: 
                          This is for illustration purposes only. The synthetic data was generated using ChatGPT.</span>")

caption_text <- str_glue("{tt} {li} stevenponce &bull; {mn} @sponce1(graphic.social) &bull; {gh} poncest &bull; #rstats #ggplot2")

note_text <- str_glue("**1% Surviving Rate**")
...
plot <- plot +
  # Adding manual labels using geom_text
  geom_text(
    aes(
      x = data$label_x, y = data$label_y,
      label = format(data$molecules, big.mark = ","),
      hjust = data$hjust_value
    ),
    hjust = 0.5
  ) +

  # Adding a descriptive text at a specific position
  annotate(
    "text",
    x = 0, y = 1,
  ...
  ggplot2::geom_curve(
    aes(x = 0.26, xend = 0.16, y = "Compound\nScreening", yend = "Compound\nScreening"),
    curvature = 0.4,
...
# ggfunnel    * 0.1.0      2024-06-01 [1] Github (pedropark99/ggfunnel@cd890d8)
