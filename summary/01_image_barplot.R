...
## Challenge: #SWDchallenge 2023 
## Author:    Steven Ponce
## Date:      2023-12-08

## Goal:      Generate a bar plot summary using images of my #SWDchallenge contributions
...
images_df <- data.frame(images_list) %>% 
  ...
  # add year, count columns & filter out unwanted images
  mutate(
    year  = str_extract(image_path, "[0-9]{4}"),
    year  = factor(year),
    count = 1,
    action = case_when(
      str_detect(image_path, "\\original") | str_detect(image_path, "version_02.png") ~ "remove",
      TRUE               ~ "keep"
    )
  ) %>% 
  filter(action == "keep") %>% 
...
title_text    <- str_glue("#STWD Challenge Contributions") 
